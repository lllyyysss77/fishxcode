...
| 分组 | 分组行、令牌行 | 请求所属分组，例如套餐分组、默认分组或特定模型分组 | 判断问题是否集中在某个套餐、模型或上游资源池 |
| 用户ID | 令牌行 | 使用该令牌的用户 ID | 企业排查时用于定位成员账号 |
| 用户名 | 令牌行 | 使用该令牌的用户名 | 用于团队报表、成员沟通和权限核对 |
| 令牌 | 令牌行 | 控制台中配置的 Token 名称 | 判断异常是否只集中在某个 Token |
| 成功率 | 分组行、令牌行 | 成功请求占比 | 成功率明显低于同组其他行时，优先排查该分组或令牌 |
| 请求数 | 分组行、令牌行 | 当前时间范围内的总请求数 | 样本量太小时不要过度解读成功率 |
| 成功 | 分组行、令牌行 | 成功请求数 | 与“请求数”“错误”一起判断整体可用性 |
| 错误 | 分组行、令牌行 | 失败请求数 | 错误数升高时，优先查看“失败原因”和错误日志 |
| 消耗 | 分组行、令牌行 | 当前时间范围内的计费消耗，导出时按金额展示 | 用于团队成本核算、项目分摊和异常消耗识别 |
| 缓存命中率 | 分组行、令牌行 | 命中缓存的 Token 占比 | 命中率低时，检查上下文是否频繁变化或无法复用 |
| 缓存Tokens | 分组行、令牌行 | 命中缓存的 Token 数 | 用于判断缓存实际节省规模 |
| 缓存请求数 | 分组行、令牌行 | 发生缓存命中的请求数 | 衡量有多少请求实际用到了缓存 |
| 缓存请求占比 | 分组行、令牌行 | 缓存请求数占总请求数的比例 | 判断缓存覆盖面，而不是只看缓存 Token 规模 |
| 平均缓存Tokens | 分组行、令牌行 | 平均每次请求命中的缓存 Token 数 | 用于比较不同成员、服务或分组的缓存利用效率 |
| 平均耗时 | 分组行、令牌行 | 平均请求耗时，单位为秒 | 耗时升高时排查长上下文、长输出、工具链路和上游响应慢 |
| 开始时间 | 分组行、令牌行 | 当前统计周期内首次请求时间 | 定位问题或调用开始时间 |
| 最后请求 | 分组行、令牌行 | 当前统计周期内最后一次请求时间 | 判断问题或调用是否仍在持续 |
| 失败原因 | 分组行 | 高频错误原因和次数；无错误时为空或 `-` | 优先处理次数最多的错误，不要只看最后一条日志 |
